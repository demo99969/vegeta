...
std::string http_get(const std::string& url) {
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "secp-worker-montgomery/3.0");
...
void print_help(const char* prog) {
    std::cout << "Usage: " << prog << " [options]\n"
              ...
              << "  --puzzle <id>            Puzzle ID (e.g. 71)\n"
              << "  --workers <count|max>    CPU threads (default: max)\n"
              << "  --api-base <url>         API server endpoint\n"
              << "  --help, -h               Show help\n";
}
...
int main(int argc, char* argv[]) {
    ...
    while (g_running.load()) {
        ...
        if (total_keys_count > 0 && slice_size > total_keys_count / (threads * 4)) {
            slice_size = std::max((uint64_t)BATCH_SIZE, total_keys_count / (threads * 4));
        }

        std::cout << "[START] Block " << rng.block << " | Range #" << rng.range_idx
                  << " | Keys: " << format_commas(total_keys_count)
                  << " (" << std::fixed << std::setprecision(2) << ((double)total_keys_count / 1e6) << " Mkeys)"
                  << " | " << threads << " thread(s)\n" << std::flush;
        ...
        std::cout << "[DONE] Block " << rng.block << " | Range #" << rng.range_idx
                  << " | Time: " << std::fixed << std::setprecision(2) << elapsed << "s"
                  << " | Speed: " << format_commas((uint64_t)speed) << " keys/s"
                  << " | Keys: " << format_commas(checked) << "\n" << std::flush;
        ...
        if (!post_ok) {
            std::cout << "[WARN] Failed to post result to server\n" << std::flush;
        }
